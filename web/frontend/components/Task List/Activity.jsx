...
import { NewTaskActivityGetService } from "../services/NewTaskService";
...
function Activity(props) {
    ...
    const handlePageChange = async () => {
        try {
            const result = await NewTaskActivityGetService(props.new_task_id, itemsPerPage, sort, sortOrder, searchField, searchData, currentPage);
            setData(result.data)
    ...
    const handleTaskUser = async (type, e) => {
        setUser(e);
        setAction();
        setTime();
        const result = await NewTaskActivityGetService(props.new_task_id, itemsPerPage, sort, sortOrder, type, e, currentPage);
        setData(result.data)
        setActivity(result.data.data)
        setSearchField(type)
        setSearchData(e)
    }
    const handleTaskAction = async (type, e) => {
        setAction(e);
        setUser();
        setTime();
        const result = await NewTaskActivityGetService(props.new_task_id, itemsPerPage, sort, sortOrder, type, e, currentPage);
    ...
    const handleTaskTime = async (type, e) => {
        setTime(e);
        setAction();
        setUser();
        const result = await NewTaskActivityGetService(props.new_task_id, itemsPerPage, sort, sortOrder, type, e, currentPage);
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <TextField
                                    value={user}
                                    onChange={(e) => handleTaskUser("user", e)}
                                    autoComplete="off"
                                />
                            </td>
                            <td>
                                <TextField
                                    value={action}
                                    onChange={(e) => handleTaskAction("action", e)}
                                    autoComplete="off"
                                />
                            </td>
                            <td>
                                <TextField
                                    value={time}
                                    onChange={(e) => handleTaskTime("time", e)}
                                    autoComplete="off"
                                />
                            </td>
                        </tr>
